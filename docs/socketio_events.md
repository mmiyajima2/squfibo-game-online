...
新しい対戦部屋を作成します。**ホストはこのイベントで自動的に部屋に参加します。**
...
**動作:**
- 部屋を作成し、ホストとゲスト用のURLを生成
- **ホストを自動的にSocket.IOルームに参加させる**
- ホストは`roomCreated`イベント受信後、すぐにリアルタイム通信が可能
...
既存の対戦部屋に**ゲストとして**参加します。

> **注意:** このイベントはゲスト専用です。ホストは`createRoom`時に自動的に部屋に参加するため、`joinRoom`を呼ぶ必要はありません。
...
**レスポンス:** `roomJoined` イベント（本人）+ `playerJoined` イベント（ホストに通知）

**動作:**
- ゲストを部屋に参加させる
- ゲストをSocket.IOルームに参加させる
- ホストに`playerJoined`イベントで通知

**エラーケース:**
- 部屋が存在しない: `ROOM_NOT_FOUND`
- 部屋が満員: `ROOM_FULL`
- 部屋が参加可能な状態ではない: `ROOM_NOT_AVAILABLE`
...
部屋作成が完了し、ホスト・ゲスト用のURLが生成されました。**ホストはこのイベント受信時点で既に部屋に参加しています。**
...
ゲストが部屋に参加しました（ホストに通知）。

> **注意:** このイベントはゲストが`joinRoom`した時にホストに送信されます。

**ペイロード:**
```typescript
{
  playerId: string;     // 参加したゲストのプレイヤーID
  playerName: string;   // 参加したゲストの名前
  role: 'guest';        // ロール（常に'guest'）
}
```

### 2.2.1 `roomJoined`

ゲスト本人が部屋に参加した確認（ゲスト本人に通知）。

**ペイロード:**
```typescript
{
  roomId: string;       // 部屋ID
  playerId: string;     // 自分のプレイヤーID
  role: 'guest';        // ロール（常に'guest'）
  roomInfo: {
    hostPlayerName: string;           // ホストの名前
    guestPlayerName: string | null;   // ゲストの名前（自分）
...
**設計のポイント:**
- ホストは`createRoom`で自動的に部屋に参加（`joinRoom`不要）
- ゲストは`joinRoom`で明示的に部屋に参加
- ゲスト参加時、ホストには`playerJoined`、ゲストには`roomJoined`が送信される
...
- **v1.1.0** (2026-02-08): 部屋参加の設計変更
  - ホストは`createRoom`で自動的に部屋に参加するように変更
  - `joinRoom`はゲスト専用に変更
  - `roomJoined`イベントを追加（ゲスト本人への確認）
  - 接続フローを更新
